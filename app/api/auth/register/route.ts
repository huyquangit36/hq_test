...
export async function POST(req: Request) {
  try {
    // Nhận dữ liệu từ Frontend (v0 thường gửi firstName và lastName riêng)
    const body = await req.json();
    console.log("Dữ liệu nhận được:", body); // Dòng này để kiểm tra trong Terminal

    const { email, password, firstName, lastName } = body;

    // Kiểm tra xem có thiếu ô nào không
    if (!email || !password || !firstName || !lastName) {
      return NextResponse.json({ error: "Vui lòng điền đầy đủ thông tin" }, { status: 400 });
    }

    // Ghép tên lại để lưu vào cột full_name trong Database
    ...
    // 3. Lưu vào bảng users (PostgreSQL local)
